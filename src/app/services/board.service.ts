...
export class BoardService {
  ...
  dropTask(fromColumnId: string, toColumnId: string, fromIndex: number, toIndex: number): void {
    if (fromColumnId === toColumnId) {
      ...
      this._columns.update(cols => {
        const task = cols.find(c => c.id === fromColumnId)?.tasks[fromIndex];
        if (!task) return cols;
        return cols.map(col => {
          if (col.id === fromColumnId)
            return { ...col, tasks: col.tasks.filter((_, i) => i !== fromIndex) };
